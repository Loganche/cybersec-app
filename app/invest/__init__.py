...
async def get_accounts():
    async with AsyncClient(token=TOKEN, sandbox_token=TOKEN_SANDBOX, target=INVEST_GRPC_API_SANDBOX) as client:
        return await client.users.get_accounts()

async def get_info():
    async with AsyncClient(token=TOKEN, sandbox_token=TOKEN_SANDBOX, target=INVEST_GRPC_API_SANDBOX) as client:
        return pprint(await client.users.get_info())

async def get_user_tariff():
    async with AsyncClient(token=TOKEN, sandbox_token=TOKEN_SANDBOX, target=INVEST_GRPC_API_SANDBOX) as client:
        return pprint(await client.users.get_user_tariff())
